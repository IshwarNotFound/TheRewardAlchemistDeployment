...
# -*- coding: utf-8 -*-
# === Imports ===
import os         # For checking file/directory existence and getting environment variables
import requests   # For downloading the ZIP file from the URL
import zipfile    # For extracting the ZIP file
import shutil     # (Optional, but can be useful for more complex file operations if needed)
  # Already likely present
# ... other existing imports like Flask, torch, pandas, etc.
import pandas as pd
import numpy as np
import os
import joblib
...
def load_model_and_data():
    # <<< Keep global variable declarations >>>
    ...
    # --- Try block starts here in your original code ---
    try:
        # --- <<< INSERT NEW: Artifact Download Logic >>> ---
        artifacts_dir = PREPROCESSED_DIR # e.g., "preprocessed_data_gnn"
        required_files = [ # List the essential files inside the artifacts dir
            MODEL_ARTIFACTS_FILE,
            CAMPAIGNS_PREPROCESSED_FILE,
            GRAPH_DATA_FILE
        ]
        # Check if the directory AND all required files exist
        artifacts_exist = os.path.exists(artifacts_dir) and \
                          all(os.path.exists(os.path.join(artifacts_dir, f)) for f in required_files)

        if not artifacts_exist:
            logger.warning(f"[{func_name}] Artifacts not found locally in '{artifacts_dir}'. Attempting download...")
            # Get the download URL from an environment variable (set later in Koyeb)
            ...
            try:
                # Download the file
                logger.info(f"[{func_name}] Downloading from URL (starts with): {artifact_zip_url[:50]}...")
                response = requests.get(artifact_zip_url, stream=True, timeout=300) # Add timeout
                ...
                raise SystemExit(f"Failed to download artifacts: {e}") from e
            ...
            if download_success:
                ...
                try:
                    ...
                    artifacts_exist = os.path.exists(artifacts_dir) and \
                                      all(os.path.exists(os.path.join(artifacts_dir, f)) for f in required_files)
                    if not artifacts_exist:
                         logger.error(f"[{func_name}] Artifacts still not found after extraction. Check ZIP contents and structure. It must contain the '{artifacts_dir}' folder with the required files inside.")
                         raise SystemExit("Artifact extraction failed or ZIP structure incorrect.")

                except zipfile.BadZipFile:
                     logger.error(f"[{func_name}] Error: Downloaded file is not a valid ZIP file.")
                     raise SystemExit("Downloaded artifact file is corrupted or not a ZIP.")
                except Exception as e:
                     logger.error(f"[{func_name}] Failed to extract artifacts: {e}")
        ...
        # --- <<< END NEW: Artifact Download Logic >>> ---

        # --- Now, continue with the original loading logic ---
        # Ensure paths used here correctly point inside the artifacts_dir
        logger.info(f"[{func_name}] Loading SBERT model...")
        sbert_model = SentenceTransformer(SBERT_MODEL_NAME, device=DEVICE)
        # ... (rest of your existing load_model_and_data function, ensuring paths like artifact_path, campaign_path, graph_path use os.path.join(artifacts_dir, ...)) ...
        artifact_path = os.path.join(artifacts_dir, MODEL_ARTIFACTS_FILE)
        # ... etc ...

    # --- Keep the original except and finally blocks ---
    except FileNotFoundError as fnf_e:
         logger.error(f"[{func_name}] ERROR: Required file not found after check/download."); logger.error(fnf_e); raise SystemExit(f"Critical file missing: {fnf_e}") from fnf_e
    except Exception as e:
         logger.error(f"[{func_name}] Error during loading (after potential download): {e}", exc_info=True); raise SystemExit(f"Failed to initialize: {e}") from e
# === Prediction Function ===
@torch.no_grad()
