...
Requires:
    ...
    - Pre-trained artifacts generated by main.py located in the
      base directory (or downloaded via ARTIFACT_URL): # <-- UPDATED DOCSTRING COMMENT
...
# ====================== CHANGE APPLIED HERE ======================
# The artifacts are expected directly in BASE_DIR after extraction
...
# Corrected version from previous SyntaxError fix
...
def load_model_and_data():
    ...
    try:
        ...
        artifacts_dir = PREPROCESSED_DIR # Now points to BASE_DIR
        required_files = [ MODEL_ARTIFACTS_FILE, CAMPAIGNS_PREPROCESSED_FILE, GRAPH_DATA_FILE ]
        # full_paths will now point directly into BASE_DIR
        full_paths = {f: os.path.join(artifacts_dir, f) for f in required_files}
        # Check if the files exist directly in BASE_DIR
        artifacts_exist = all(os.path.exists(p) for p in full_paths.values())

        if not artifacts_exist:
            logger.warning(f"[{func_name}] Artifacts not found locally in '{artifacts_dir}'. Attempting download...") # Log message is still informative
            ...
            if download_success:
                ...
                try:
                    ...
                    logger.info(f"[{func_name}] Extraction complete. Artifacts should now be in '{extract_to_dir}'.") # Updated log message
                    # Re-check if the files exist directly in BASE_DIR after extraction
                    artifacts_exist = all(os.path.exists(p) for p in full_paths.values())
                    if not artifacts_exist:
                         logger.error(f"[{func_name}] Artifact files still not found directly in '{extract_to_dir}' after extraction. Check ZIP contents.")
                         # Log the files actually present in the base directory for debugging
                         if os.path.exists(extract_to_dir):
                             logger.error(f"Files found in {extract_to_dir}: {os.listdir(extract_to_dir)}")
                         raise SystemExit("Artifact extraction failed or required files missing post-extraction.")
             ...
             logger.info(f"[{func_name}] Found existing artifacts in '{artifacts_dir}'. Skipping download.") # Log message is okay
...
try:
    ...
    raise SystemExit(f"Error loading artifact with joblib: {load_err}")
        ...
        # full_paths[GRAPH_DATA_FILE] now correctly points to the .pt in BASE_DIR
        ...
        # Custom unpickler already mapped tensors to CPU during load.
        # state_dicts should be directly usable now.
        ...
        logger.info(f"[{func_name}] Application Initialization Complete.")
         ...
         # This error should now be less likely if download/extraction works and PREPROCESSED_DIR is correct
...
def predict_campaigns_gnn_flask(user_data: dict):
    ...
    # This check is crucial, especially if global loading failed silently
    if not all([loaded_artifacts, campaigns_df_preprocessed is not None, model, predictor, sbert_model, graph_node_features is not None, graph_edge_index is not None]):
        logger.error(f"[{func_name}] Critical server resources not ready/loaded. Aborting prediction. Check initialization logs.")
...
def _get_dropdown_options():
    ...
    try:
        ...
        # This check is now even more critical because failure happens earlier if loading fails
        if loaded_artifacts is None:
             # Add more specific logging if this check fails
             logger.error("Failed in _get_dropdown_options: loaded_artifacts is None. Initialization likely failed.")
             raise RuntimeError("Model artifacts not loaded. Initialization may have failed.")
        ...
        location_encoder = loaded_artifacts.get('location_encoder') # Safe to use .get() now
        ...
        for artifact_key, option_key in encoder_map.items():
             encoder = loaded_artifacts.get(artifact_key) # Safe to use .get() now
        ...
        # Log the specific error encountered when trying to access artifacts
        logger.error(f"Error populating dropdown options (likely due to artifact access): {e}", exc_info=True)
        error_msg = "Error loading some dropdown options. Using default values. Check server logs." # More informative error
...
def predict():
    ...
    try:
        ...
        if form_ok:
            ...
            # Call the prediction function which now has a better pre-check
...
if __name__ == "__main__":
    ...
    # The global scope loading happens before this check, but this warning is still useful
    if model is None or predictor is None or loaded_artifacts is None:
         logger.warning("!!! Models/artifacts may not have loaded correctly during global init. Check logs above. !!!")
