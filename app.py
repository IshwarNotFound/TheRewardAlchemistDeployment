...
Requires:
    ...
    - Pre-trained artifacts generated by main.py located in the
      project root directory (or downloaded via ARTIFACT_URL):  <-- Adjusted expectation
...
import pandas as pd
import numpy as np
import joblib # Used directly now for loading, also used indirectly by unpickler
...
# === Imports for Custom Unpickler (Kept in case needed later) ===
import pickle
import io
# ====================================
...
# Kept for reference but not currently used by default
...
def load_model_and_data():
    ...
    try:
            ...
            if download_success:
                ...
                try:
                    ...
                    with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                        # Check contents before extraction (optional debug)
                        # logger.debug(f"ZIP contents: {zip_ref.namelist()}")
                    ...
                    if not artifacts_exist:
                         ...
                         # Check if the subdirectory was accidentally created
        ...
        # === TEMPORARY CHANGE: Use joblib.load instead of CPU_Unpickler ===
        ...
        try:
            ...
            logger.error(f"[{func_name}] File not found during joblib.load: {fnf_e}")
            raise SystemExit(f"File not found during joblib.load: {fnf_e}") from fnf_e
        except Exception as load_err:
            logger.error(f"[{func_name}] Error during joblib.load: {load_err}", exc_info=True)
            raise SystemExit(f"Error loading artifact with joblib: {load_err}") from load_err
        # === END of TEMPORARY replacement block ===
        ...
        # Load state dicts (should be on CPU already if saved correctly or via CPU_Unpickler if used)
