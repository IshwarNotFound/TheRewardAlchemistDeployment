...
Requires:
    ...
    - Pre-trained artifacts generated by main.py located in the
      project root directory (or downloaded via ARTIFACT_URL):
        - recommender_gnn_artifacts_hackathon_final_v2.pkl (MUST contain a dictionary)
...
import pandas as pd
import numpy as np
import joblib # Used indirectly by custom unpickler for some objects
...
# Corrected: Artifacts are now expected directly in BASE_DIR after extraction
PREPROCESSED_DIR = BASE_DIR
...
# This is CRUCIAL for loading GPU-trained models on CPU
class CPU_Unpickler(pickle.Unpickler):
    def find_class(self, module, name):
        if module == 'torch.storage' and name == '_load_from_bytes':
            # Intercept the loading of storage bytes
            # and map them to CPU using torch.load
...
def load_model_and_data():
    ...
    try:
        ...
        artifacts_dir = PREPROCESSED_DIR # Now expects artifacts directly in BASE_DIR
        ...
        # Check if the *files* exist in the artifacts_dir (which is BASE_DIR)
        artifacts_exist = all(os.path.exists(p) for p in full_paths.values())
        ...
        if not artifacts_exist:
            ...
            # --- Extraction Logic ---
            if download_success:
                extract_to_dir = BASE_DIR # Extract directly into the base directory
                try:
                    logger.info(f"[{func_name}] Extracting {zip_path} to '{extract_to_dir}'...")
                    with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                        zip_ref.extractall(extract_to_dir)
                    logger.info(f"[{func_name}] Extraction complete. Artifacts should now be in '{extract_to_dir}'.")
                    # Re-check if files exist in the target directory (BASE_DIR)
                    artifacts_exist = all(os.path.exists(p) for p in full_paths.values())
                    if not artifacts_exist:
                         logger.error(f"[{func_name}] Artifact files still not found directly in '{extract_to_dir}' after extraction.")
                         logger.error(f"Files found in {extract_to_dir}: {os.listdir(extract_to_dir)}")
                         if os.path.exists(os.path.join(extract_to_dir, "preprocessed_data_gnn")):
                             logger.error(f"ERROR: Found unexpected subdirectory 'preprocessed_data_gnn' after extraction. Check ZIP structure!")
        ...
        # === Reverted CHANGE: Use CPU_Unpickler again ===
        pkl_file_path = full_paths[MODEL_ARTIFACTS_FILE] # Get the path
        logger.info(f"[{func_name}] Attempting to load main artifacts from '{pkl_file_path}' using custom CPU_Unpickler...")
        try:
            # Check if file exists before attempting to load
            if not os.path.exists(pkl_file_path):
                raise FileNotFoundError(f"File not found at the specified path: {pkl_file_path}")

            # Use the custom unpickler which handles map_location='cpu'
            with open(pkl_file_path, 'rb') as f:
                loaded_artifacts = CPU_Unpickler(f).load()

            # Add extra logging right after loading
            logger.info(f"Successfully loaded object using CPU_Unpickler. Type: {type(loaded_artifacts)}")

            # <<< IMPORTANT CHECK: Verify loaded_artifacts is a dictionary >>>
            if not isinstance(loaded_artifacts, dict):
                logger.error(f"[{func_name}] CRITICAL: Loaded artifact from PKL is type '{type(loaded_artifacts)}', expected 'dict'.")
            ...
            logger.error(f"[{func_name}] File not found during CPU_Unpickler load: {fnf_e}")
            raise SystemExit(f"File not found during CPU_Unpickler load: {fnf_e}") from fnf_e
        except Exception as load_err:
            logger.error(f"[{func_name}] Error during CPU_Unpickler load: {load_err}", exc_info=True)
            # Check specifically for the CUDA error, though CPU_Unpickler should prevent it
            if "Attempting to deserialize object on a CUDA device" in str(load_err):
                logger.critical("CPU_Unpickler failed to map CUDA Tensors. Check its implementation.")
            raise SystemExit(f"Error loading artifact with CPU_Unpickler: {load_err}") from load_err
        ...
        # IMPORTANT: Make sure 'loaded_artifacts' is the dictionary here
        if not isinstance(loaded_artifacts, dict):
             logger.critical(f"Cannot initialize GNN/Predictor because loaded_artifacts is type {type(loaded_artifacts)}, not dict.")
             raise SystemExit("Cannot proceed without dictionary artifact.")

        num_node_features = graph_node_features.shape[1]
        gnn_hidden_channels = loaded_artifacts.get('gnn_hidden_channels', 128) # Use .get for safety
        gnn_output_channels = loaded_artifacts.get('gnn_output_channels', 64) # Use .get for safety
        model = GraphSAGE_GNN(num_node_features, gnn_hidden_channels, gnn_output_channels)
        predictor = LinkPredictor(gnn_output_channels)

        # Check if state dict keys exist before loading
        model_state_key = 'model_state_dict'
        predictor_state_key = 'predictor_state_dict'
        if model_state_key not in loaded_artifacts:
            raise SystemExit(f"Missing key '{model_state_key}' in loaded artifacts dictionary.")
        if predictor_state_key not in loaded_artifacts:
             raise SystemExit(f"Missing key '{predictor_state_key}' in loaded artifacts dictionary.")

        # Load state dicts (CPU_Unpickler should have mapped them to CPU during the main load)
        model.load_state_dict(loaded_artifacts[model_state_key])
        predictor.load_state_dict(loaded_artifacts[predictor_state_key])
...
def predict_campaigns_gnn_flask(user_data: dict):
    ...
    if not all([loaded_artifacts, campaigns_df_preprocessed is not None, model, predictor, sbert_model, graph_node_features is not None, graph_edge_index is not None]):
        ...
        # Also check if loaded_artifacts is specifically a dict here, as it might be None or wrong type
        if not isinstance(loaded_artifacts, dict):
             logger.error(f"[{func_name}] loaded_artifacts is not a dictionary (type: {type(loaded_artifacts)}). Cannot proceed.")
             return [], 0.0, "Error: Server resources (artifacts dictionary) are not ready. Please try again later or contact support."
    ...
    try:
        ...
        for col in user_defaults_pred_scalar:
            if col not in user_df.columns: user_df[col] = user_defaults_pred_scalar[col]
            else: user_df[col].fillna(user_defaults_pred_scalar[col], inplace=True)
        ...
        # Ensure loaded_artifacts is a dict before accessing keys
        if not isinstance(loaded_artifacts, dict): raise TypeError("loaded_artifacts is not a dictionary during prediction.")
        ...
        for original_campaign_id in campaign_ids_in_order:
            ...
            try:
                ...
                user_interest_reshaped = user_interest_emb_np.reshape(1, -1)
                user_watch_reshaped = user_watch_emb_np.reshape(1, -1)
                camp_promo_reshaped = camp_promo_embed.reshape(1, -1) if camp_promo_embed is not None else None
                camp_cat_reshaped = camp_cat_embed.reshape(1, -1) if camp_cat_embed is not None else None

                if user_interest_reshaped.size > 1 and camp_promo_reshaped is not None and camp_promo_reshaped.size > 1 and user_interest_reshaped.shape[1] == camp_promo_reshaped.shape[1]:
                    interest_promo_sim = cosine_similarity(user_interest_reshaped, camp_promo_reshaped)[0][0]
                if user_watch_reshaped.size > 1 and camp_cat_reshaped is not None and camp_cat_reshaped.size > 1 and user_watch_reshaped.shape[1] == camp_cat_reshaped.shape[1]:
                     watch_cat_sim = cosine_similarity(user_watch_reshaped, camp_cat_reshaped)[0][0]
...
def _get_dropdown_options():
    ...
    error_msg = None
    try:
        # === IMPORTANT: Check if loaded_artifacts is a dictionary before using .get() ===
        if loaded_artifacts is None:
             # Don't raise immediately, check type first
             logger.warning("Model artifacts are None during _get_dropdown_options.")
             raise RuntimeError("Model artifacts not loaded.") # Keep original error for consistency

        if not isinstance(loaded_artifacts, dict):
            logger.error(f"Loaded artifacts is type {type(loaded_artifacts)}, expected dict. Cannot populate dropdowns.")
            raise RuntimeError("Loaded artifacts are not in the expected dictionary format.")
        # =============================================================================

        # Now safe to assume loaded_artifacts is a dictionary
        ...
        else: logger.warning("Encoder 'location_encoder' not found/invalid in artifacts.")
        ...
        logger.error(f"Error populating dropdown options (likely due to artifact access): {e}", exc_info=True)
        error_msg = "Error loading some dropdown options. Using default values or check server logs." # More specific error
...
def predict():
    ...
    if error_msg_opts:
        logger.warning("Dropdown options failed to load (likely init failure), returning error on predict request.")
        # Pass the specific error message from _get_dropdown_options
...
if __name__ == "__main__":
    ...
    # Check if essential globals are loaded after initialization attempt
    if model is None or predictor is None or loaded_artifacts is None or not isinstance(loaded_artifacts, dict):
         logger.warning("!!! Models/artifacts may not have loaded correctly. Check logs above for initialization errors. !!!")
